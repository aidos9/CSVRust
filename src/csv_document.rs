...
impl CSVDocument {
    ...
    {
        return CSVDocument {header: csv_row::CSVRow::new(), contents:vec![]};
    }
    ...
    pub fn parse_string(input: &String) -> Result<CSVDocument, &'static str>
    {
        ...
        return Ok(CSVDocument{header: header_row, contents: contents_rows});
    }
}
...
mod tests {
    ...
    fn test_parse_string_embedded_quotes()
    {
        ...
        assert_eq!(doc.header.cells, vec!["cats","dogs","cars","humans"]);
        assert_eq!(doc.contents.len(), 1);
        assert_eq!(doc.contents[0].cells, vec!["none","\"james\"","none","none"]);
    }
    ...
    fn test_parse_string_embedded_quotes_2()
    {
        ...
        assert_eq!(doc.contents.len(), 2);
        assert_eq!(doc.contents[0].cells, vec!["none","\"james\"","none","none"]);
        assert_eq!(doc.contents[1].cells, vec!["james,\"cars!\"","none","none","none"]);
    }
    ...
    fn test_parse_string_embedded_nl()
    {
        ...
        assert_eq!(doc.header.cells, vec!["cats","dogs","cars","humans"]);
        assert_eq!(doc.contents.len(), 1);
        assert_eq!(doc.contents[0].cells, vec!["none","james,\njohn\n catherine!","none","none"]);
    }
